...
This method is then preferred over other variance-based feature importance approaches because it does not rely on regression which suffers from constraints described in [Statistical considerations]. In addition, the sample sizes are small (40-41) and 
...
Similar to the variance decomposition, the transform is applied independently to series of yearly maxima TX1day events at every location on Earth with the aim of providing insights in spatial variability. PCA is preferred over machine-learning approaches such as .. due to our small sample size at each location. 
